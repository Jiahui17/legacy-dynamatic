...
GraphToVerilog::GraphToVerilog(std::string filen){
	...
	std::cout << "[TEST] finished dotReader(filen)" << endl;
	dotReader.lineReader();
	std::cout << "[TEST] finished lineReader(filen)" << endl;
	this->dotReader = dotReader;
...
void GraphToVerilog::writeToFile(){
	...
	std::cout << "[TEST] finished getFileName()" << endl;
...
void GraphToVerilog::writeVerilogCode(){
	std::cout << "[TEST] starting writeVerilogCode()" << endl;

	verilogCode += writeTopModuleName();

	std::cout << "[TEST] finished writeTopModuleName()" << endl;

	insertTab();
	verilogCode += writeTopModulePorts();
	removeTab();

	std::cout << "[TEST] finished writeTopModulePorts()" << endl;

	insertTab();
	verilogCode += writeModulePortWires();
	removeTab();

	std::cout << "[TEST] finished writeModulePortWires()" << endl;
	...
	std::cout << "[TEST] finished writeInputOutputConnections()" << endl;
	...
	std::cout << "[TEST] finished writeModuleInstantiation()" << endl;
...
std::string GraphToVerilog::writeModulePortWires(){
	...
	for(auto it = dotReader.getComponentList().begin(); it != dotReader.getComponentList().end(); it++){
		std::cout << "[TEST] Writing the port wires of component: " << (*it)->name << std::endl;
	...
	for(auto it = dotReader.getComponentList().begin(); it != dotReader.getComponentList().end(); it++){
		if((*it)->type == COMPONENT_START){
			...
			if((*it)->op == OPERATOR_ADD){
				...
				moduleInstances += "\n\n";
				std::cout << "[WARN] Not handled operator type: " << (*it) -> op << ", using default module instantiation method." << std::endl;
			}
	...
	for(auto it = dotReader.getComponentList().begin(); it != dotReader.getComponentList().end(); it++){
		if((*it)->type == COMPONENT_START){
			...
			if((*it)->op == OPERATOR_ADD){
				...
				inputoutput += "\n";
				std::cout << "[WARN] Not handled operator type: " << (*it) -> op << ", using default port connection method." << std::endl;
			}
		} else if((*it)->type == COMPONENT_SOURCE){
