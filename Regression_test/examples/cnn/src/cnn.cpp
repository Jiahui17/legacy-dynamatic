...
void cnn(in_int_t input[inputSize], in_int_t bias[biasSize], in_int_t weight[weightSize], inout_int_t C[CSize], out_int_t output[outputSize]){
...
int main(void){
	...
	for(int i = 0; i < AMOUNT_OF_TEST; ++i){
		cnn(input[i], bias[i], weight[i], C[i], output[i]);
	}
