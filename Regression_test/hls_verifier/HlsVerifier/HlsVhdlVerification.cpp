...
namespace hls_verify {
	...
	void generate_modelsim_scripts(const VerificationContext& ctx) {
		...
		// Jiahui 05/08/2023: Some floating point library compiled
		// using a non-free version of questasim might need work.glbl.
		// So we check if filelist_verilog contains glbl.v or not
		bool glbl_module_found = false;
		for (string filename : filelist_verilog) {
			cout << "[TEST] Verilog file name: " << filename << endl;
			if (filename.compare("glbl.v") == 0) {
				cout << "[TEST] Found glbl.v" << endl;
				glbl_module_found = true;
			}
		}
		...
		sim << "eval vsim " << ctx.get_vhdl_duv_entity_name();
		// Jiahui 05/08/2023: here i am checking if i need to compile an extra glbl.v module for some floating point libraries
		if (glbl_module_found) {
			sim << "_tb work.glbl" << endl;
		} else {
			sim << "_tb" << endl;
		}
		sim << "run -all" << endl;
